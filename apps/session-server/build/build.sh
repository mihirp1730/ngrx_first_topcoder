...
echo "Run lint"
npm run nx -- run session-server:lint \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Run test"
npm run nx -- run session-server:test \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Cleanup LCOV file"
# Change paths in lcov.info to be relative and not absolute
CDIR="SF:$(pwd)/"
sed -i "s|$CDIR|SF:|g" coverage/apps/session-server/lcov.info

echo "Run build"
npm run nx -- run session-server:build:production \
  || { echo '##vso[task.logissue type=error]Production build failed'; exit 1; }

echo "Creating artifacts"
bash apps/session-server/build/deploy/create-artifacts.sh
