...
export IN_DIR="$BUILD_REPOSITORY_LOCALPATH/apps/gateway-server/build/deploy"
export OUT_DIR="$BUILD_REPOSITORY_LOCALPATH/buildartifacts/gateway-server"
mkdir -p $OUT_DIR
...
then
    ...
    docker build -f $BUILD_REPOSITORY_LOCALPATH/apps/gateway-server/build/dist.Dockerfile -t $DOCKER_IMAGE .
