...
export class MapLargeHelperService {
  ...
  public getActiveTables(connectionInfo: IMlConnectionInfo): Observable<any> {
    const headers = {
      'Content-Type': 'application/json',
      mltoken: connectionInfo.accessToken,
      mlUser: connectionInfo.userId
    };
    return this.httpClient.get<any>(`${connectionInfo.baseUrl}/Remote/GetActiveTables?verbose=true`, { headers });
  }

  public getOpportunitiesOnLoad(connectionInfo: IMlConnectionInfo, tableName: string): Observable<any> {
    const req = {
      action: 'table/query',
      query: {
        engineVersion: 0,
    ...
    const body = new HttpParams().set('uParams', 'action:table%2Fquery;fallbackToPoll:true').set('request', JSON.stringify(req));
    const headers = {
      'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
      mltoken: connectionInfo.accessToken,
      mlUser: connectionInfo.userId
    };
    return this.httpClient.post<any>(`${connectionInfo.baseUrl}/Api/ProcessDirect`, body, { headers });
  }
}
