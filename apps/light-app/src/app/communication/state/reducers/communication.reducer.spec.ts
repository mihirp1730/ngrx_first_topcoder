...
describe('communicationReducer', () => {
  ...
  describe('createChat', () => {
    it('should set the state of activeChat', () => {
      const action = comunicationActions.createChat();
      const newState = communicationReducer(initialState, action);
  ...
  describe('setMetadata', () => {
    it('should set the state of metadata', () => {
      const metadata = { opportunityId: '123', opportunityName: 'test', dataVendorId: 'vendor id 1' };
      const action = comunicationActions.setMetadata({ metadata });
      const newState = communicationReducer(initialState, action);
      expect(newState.metadata).toEqual(metadata);
    });
  });

  describe('loadVendorContactDetailsSuccess', () => {
    it('should set the state with vendor contact details after success', () => {
      ...
      const action = comunicationActions.loadVendorContactDetailsSuccess({ vendorContactDetails });
      const newState = communicationReducer(initialState, action);
      expect(newState.vendorContactDetails).toEqual(vendorContactDetails);
    });
  });
