...
const _communicationReducer = createReducer(
  ...
  on(communicationActions.openChat, (state, { chatId }): State => {
    return {
      ...
      isLoadingWhileGettingMessages: true
    };
  }),
  ...
  on(communicationActions.setMetadata, (state, { metadata }): State => {
    return {
      ...state,
      metadata
    };
  }),
  on(communicationActions.loadVendorContactDetailsSuccess, (state, { vendorContactDetails }): State => {
    return {
      ...state,
