...
echo "Run lint"
npm run nx -- run light-app:lint \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Run test"
npm run nx -- run light-app:test \
...
CDIR="SF:$(pwd)/"
sed -i "s|$CDIR|SF:|g" coverage/apps/light-app/lcov.info

echo Run build
npm run nx -- run light-app:build:production \
  || { echo '##vso[task.logissue type=error]Production build failed'; exit 1; }

echo Calling create-artifacts.sh from build.sh
bash apps/light-app/build/deploy/create-artifacts.sh
