...
echo "Run lint"
npm run nx -- run auth-server:lint \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Run test"
npm run nx -- run auth-server:test \
...
CDIR="SF:$(pwd)/"
sed -i "s|$CDIR|SF:|g" coverage/apps/auth-server/lcov.info

echo "Run build"
npm run nx -- run auth-server:build:production \
  || { echo '##vso[task.logissue type=error]Production build failed'; exit 1; }

echo "Creating artifacts"
bash apps/auth-server/build/deploy/create-artifacts.sh
