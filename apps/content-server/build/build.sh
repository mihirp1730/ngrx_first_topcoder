...
echo "Run lint"
npm run nx -- run content-server:lint \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Run tests"
npm run nx -- run content-server:test \
...
CDIR="SF:$(pwd)/"
sed -i "s|$CDIR|SF:|g" coverage/apps/content-server/lcov.info

echo "Run build"
npm run nx -- run content-server:build:production \
  || { echo '##vso[task.logissue type=error]Production build failed'; exit 1; }

echo "Creating artifacts"
bash apps/content-server/build/deploy/create-artifacts.sh
