...
echo "Run lint"
npm run nx -- run storage-server:lint \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Run test"
npm run nx -- run storage-server:test \
...
CDIR="SF:$(pwd)/"
sed -i "s|$CDIR|SF:|g" coverage/apps/storage-server/lcov.info

echo "Run build"
npm run nx -- run storage-server:build:production \
  || { echo '##vso[task.logissue type=error]Production build failed'; exit 1; }

echo "Creating artifacts"
bash apps/storage-server/build/deploy/create-artifacts.sh
