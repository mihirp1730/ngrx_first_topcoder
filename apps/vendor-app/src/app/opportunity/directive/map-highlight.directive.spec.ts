...
import { MapHighlightDirective } from './map-highlight.directive';
...
@Component({
  template: ` <mat-card class="card-container" [apolloMapHighlight]="{mapReprensentationId : 'map_representation_id_0', type:'Opportunity'}"> </mat-card> `
})
class TestGisSearchResultComponent {}

describe('MapHighlightDirective', () => {
  let component: TestGisSearchResultComponent;
  let fixture: ComponentFixture<TestGisSearchResultComponent>;
  let inputEl: DebugElement;
  let directive: MapHighlightDirective;
  let settingsService: SettingsService;
  let mockStore: MockStore;

  beforeEach(() => {
    TestBed.configureTestingModule({
      declarations: [TestGisSearchResultComponent, MapHighlightDirective],
    ...
    inputEl = fixture.debugElement.query(By.directive(MapHighlightDirective));
    directive = inputEl.injector.get(MapHighlightDirective);
    settingsService = TestBed.inject(SettingsService);
    directive.selectedLayer = [{
      type: 'Opportunity',
      fileName: 'file_name_0',
      fileId: 'file_id_0',
      mapRepresentationId: 'map_representation_id_0'
  ...
  it('@HostListener : mouseleave - enable/disable highlight', () => {
    ...
    directive.highlightResultObject = { type:'Opportunity', fileId : 'file-id', fileName : "name", mapRepresentationId : "map_representation_id_0"};
    directive.settingsService.map.highlightResult = true;
  ...
  it('highlightResult() : should create new layer', () => {
    ...
    directive.gisMapLargeService.settingsService = directive.settingsService;
    directive.gisMapLargeService.settingsService.layersConfiguration = <any>[{
      "id": "Opportunity",
      "name": "Opportunity",
      "style": {
        "rules": [
      ...
      "tableInfo": {
        "table": {
          "name": "2S21QQGC6J/Asset"
        },
        "where": [
          [
            {
              "col": "mapRepresentationId",
              "test": "Equal",
              "value": "null"
            }
          ]
  ...
  it('highlightResult() : should not update style', () => {
    const mapRepId = "map_representation_id_0";
    directive.settingsService.map.highlightResult = false;
    directive.highlightResult(mapRepId);
  });
