...
echo "Run lint"
npm run nx -- run vendor-app:lint \
  || { echo '##vso[task.logissue type=error]Lint failed'; exit 1; }

echo "Run test"
npm run nx -- run vendor-app:test \
  || { echo '##vso[task.logissue type=error]Tests failed'; exit 1; }

echo "Cleanup LCOV file"
# Change paths in lcov.info to be relative and not absolute
CDIR="SF:$(pwd)/"
sed -i "s|$CDIR|SF:|g" coverage/apps/vendor-app/lcov.info

echo Run build
npm run nx -- run vendor-app:build:production \
  || { echo '##vso[task.logissue type=error]Production build failed'; exit 1; }

echo Calling create-artifacts.sh from build.sh
bash apps/vendor-app/build/deploy/create-artifacts.sh
