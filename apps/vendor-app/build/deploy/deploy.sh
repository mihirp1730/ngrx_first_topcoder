...
# Add deployment environment to configmap-environements
envsubst < $SCRIPT_DIR/environment.json  | sed 's/"/\\"/g' | sed -z 's/\n/\\r\\n/g' > $SCRIPT_DIR/environment.tmp
export DEPLOYMENT_ENVIRONMENT_CONTENT=`cat $SCRIPT_DIR/environment.tmp`
envsubst < $SCRIPT_DIR/configmap-environments.yaml > $SCRIPT_DIR/configmap-environements.tmp
mv $SCRIPT_DIR/configmap-environements.tmp $SCRIPT_DIR/configmap-environments.yaml

# Add nginx config to configmap-nginx-config
envsubst '${CSP_HEADER_VALUE}' < $SCRIPT_DIR/nginx.common.conf | sed 's/"/\\"/g' | sed -z 's/\n/\\r\\n/g' > $SCRIPT_DIR/nginx.common.conf.tmp
export DEPLOYMENT_NGINX_COMMON_CONFIG=`cat $SCRIPT_DIR/nginx.common.conf.tmp`

envsubst '${change-only-it}' < $SCRIPT_DIR/nginx.default.conf | sed 's/"/\\"/g' | sed -z 's/\n/\\r\\n/g' > $SCRIPT_DIR/nginx.default.conf.tmp
export DEPLOYMENT_NGINX_DEFAULT_CONFIG=`cat $SCRIPT_DIR/nginx.default.conf.tmp`

envsubst < $SCRIPT_DIR/configmap-nginx-config.yaml > $SCRIPT_DIR/configmap-nginx-config.tmp
mv $SCRIPT_DIR/configmap-nginx-config.tmp $SCRIPT_DIR/configmap-nginx-config.yaml
...
kubectl apply --namespace=$KUBE_NAMESPACE -f $SCRIPT_DIR/configmap-environments.yaml
kubectl apply --namespace=$KUBE_NAMESPACE -f $SCRIPT_DIR/configmap-nginx-config.yaml
